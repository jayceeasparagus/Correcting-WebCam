...
static void Servo_SetPulse(uint32_t channel, uint16_t pulse_us);
...
/* USER CODE BEGIN 2 */
  ...
  UART_SendText("\r\nSTM32 ready. Commands: PING, PAN_LEFT, PAN_RIGHT, TILT_UP, TILT_DOWN, STOP\r\n");
...
static void UART_ProcessCommand(const char *command)
{
  ...
  else if (strcmp(command, "PAN_LEFT") == 0)
  {
    UART_SendText("OK PAN_LEFT\r\n");
    BlinkStatus(2);
  }
  else if (strcmp(command, "PAN_RIGHT") == 0)
  {
    UART_SendText("OK PAN_RIGHT\r\n");
    BlinkStatus(3);
  }
  else if (strcmp(command, "TILT_UP") == 0)
  {
    UART_SendText("OK TILT_UP\r\n");
    BlinkStatus(4);
  }
  else if (strcmp(command, "TILT_DOWN") == 0)
  {
    UART_SendText("OK TILT_DOWN\r\n");
    BlinkStatus(5);
  }
  else if (strcmp(command, "STOP") == 0)
  {
    UART_SendText("OK STOP\r\n");
    HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET);
  }
  else
  {
